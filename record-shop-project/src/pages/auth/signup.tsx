...
const SignUpPage = () => {
	...
	// input references
	const inputRefs = {
		userAccount: useRef<HTMLInputElement | null>(null),
		userPassword: useRef<HTMLInputElement | null>(null),
		userName: useRef<HTMLInputElement | null>(null),
		zonecode: useRef<HTMLInputElement | null>(null),
		userPhoneNumber: useRef<HTMLInputElement | null>(null),
		userAddress: useRef<HTMLInputElement | null>(null),
		userDetailAddress: useRef<HTMLInputElement | null>(null),
	};
	...
	const onSubmitToSignUp = async (e: React.MouseEvent<HTMLButtonElement>) => {
		...
		for (const key in formData) {
			if (formData[key as keyof UserType].toString().trim() === '')
				emptyField.push(key);
		}
		...
		if (emptyField.length) {
			...
			if (
				firstEmptyField &&
				inputRefs[firstEmptyField as keyof typeof inputRefs]
			) {
				inputRefs[firstEmptyField as keyof typeof inputRefs].current?.focus();
			}
			return;
		}
